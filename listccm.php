...
$sql = "SELECT DISTINCT ip,ccmtype FROM t_data WHERE tod>=:start AND tod<=:end ORDER BY ip,ccmtype LIMIT :limit";
$sth = $dbh->prepare($sql);
$sth->execute(array(':start'=>$s,':end'=>$e,':limit'=>$l));
$ccmlist = $sth->fetchAll(PDO::FETCH_ASSOC);

echo json_encode($ccmlist);
exit;
?>
